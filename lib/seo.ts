...
// NOTE: This branch (feat/track-f-seo-og) intentionally avoids importing
// from @/lib/social and @/lib/markup so the file builds against the bare
// `main` branch. The full integration imports `MOCK_USERS` and `extractTags`
// from those modules — when Track A's social layer is merged, the safe
// pattern is the defensive `Array.isArray(MOCK_USERS) ? … : []` already
// landed by Track D. See SEO_NOTES.md "Caveats" for context.
type MockUserLite = {
  username: string;
  moments: { caption: string; location: string }[];
};
let MOCK_USERS: MockUserLite[] = [];
try {
  // Lazy require — present when Track A's social layer is merged in,
  // missing on the bare main branch. We swallow the error in either case.

  MOCK_USERS = (require("@/lib/social") as { MOCK_USERS?: MockUserLite[] })
    .MOCK_USERS ?? [];
} catch {
  MOCK_USERS = [];
}
